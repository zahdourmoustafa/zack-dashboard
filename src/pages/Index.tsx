...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fortyEightHoursAgo = new Date(Date.now() - 1 * 60 * 1000);
  ...
  const handleTogglePriority = async (orderId: string, currentPriority: boolean) => {
    ...
    try {
      const { data, error } = await supabase
        .from("orders")
        .update({ is_priority: !currentPriority, updated_at: new Date().toISOString() })
        .eq("id", orderId)
        .select(
          `
          id,
      ...
      if (data) {
        ...
        toast({
          ...
          description: `La priorité de la commande #${orderId} a été ${!currentPriority ? 'ajoutée' : 'retirée'}.`,
        });
      }
    } catch (error) {
  ...
  const handleCloneOrder = async (orderId: string) => {
    ...
    try {
      ...
      const newOrderData = {
        ...
        notes: `Clone de la commande #${originalOrder.id}${originalOrder.notes ? ' - ' + originalOrder.notes : ''}`,
      ...
      const { data: newOrderInsertResult, error: insertOrderError } =
        await supabase
          .from("orders")
          .insert({...newOrderData, is_priority: originalOrder.is_priority || false })
          .select(
            `
            id,
            order_date,
      ...
      if (originalOrder.order_items && originalOrder.order_items.length > 0) {
        ...
        // Re-fetch the new order with its items for the UI state
         const { data: newOrderWithItems, error: newOrderFetchError } = await supabase
          .from("orders")
          .select(
            `
            id,
        ...
        if (newOrderWithItems) {
             setOrders((prevOrders) => [newOrderWithItems as unknown as Order, ...prevOrders]);
        }
      ...
      toast({
        ...
        description: `Impossible de cloner la commande. ${error instanceof Error ? error.message : ''}`,
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-white min-h-screen">
      ...
      <div className="sm:hidden mb-8 space-y-5">
        ...
        <h2 className="text-lg font-semibold text-gray-700 pt-2">Création rapide</h2>
      ...
      <Tabs defaultValue="all" className="mt-0">
        ...
        <TabsContent value="reporte_annule">
          <OrderTable
